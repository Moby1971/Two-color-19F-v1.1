...
folder='L:\basic\divi\Projects\cosart\fluor\fluor\scans\CC_091117_19F_PBS_Jp1';
kspaces='kspaces110917.mat'
...
if RingingFilter
    rho=0.75;
    for nn=1:128
        Hamming(nn)=(1-rho)*0.5*(1-cos((2*pi*(nn-1)/(128))))+(rho);
    end
    
    tmp=bsxfun(@times,ones(128,128,128),Hamming);
    tmp=bsxfun(@times,tmp,permute(Hamming,[3 1 2]));
    tmp=bsxfun(@times,tmp,permute(Hamming,[2 3 1]));
...
exportfolder='L:\basic\divi\Projects\cosart\fluor\fluor\scans\CC_091117_19F_PBS_Jp1'
mkdir(exportfolder); cd(exportfolder)
params.orientation='axi'
params.fignumber=53
params.export=0
params.slicerange=50:90;
params.exportname='temp'
...
%%
cd('L:\basic\divi\Projects\cosart\fluor\fluor\recon_realdata\dec17\Recons\Mouse2_15_1_2018')
save('Mouse4_48h_ringing075.mat','Recon_originalFOV')
