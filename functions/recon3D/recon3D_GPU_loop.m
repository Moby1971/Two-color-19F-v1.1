...
kspaces='kspaces_Mouse4_48H.mat'
...
if RingingFilter
    rho=0.75;
    for nn=1:64
        Hamming(nn)=(1-rho)*0.5*(1-cos((2*pi*(nn-1)/(64))))+(rho);
    end
    
    tmp=bsxfun(@times,ones(64,64,64),Hamming);
...
if use_CG
    ...
    for outeriter=1:n_outeriter
        ...
        RCG=nl_conjgrad_fluor_3D(M, data, RCG,niter,TVOP2,lambda,[nx ny nz],0,3);
    end
