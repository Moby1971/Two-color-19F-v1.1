...
if use_CG
    ...
    for outeriter=1:10
        ...
        RCG=nl_conjgrad_fluor_3D(M, data, RCG,niter,TVOP2,lambda,[nx ny nz],1,10);
        
        k = BlindDeconvolutionOptimization(RCG,N,linear_recon,nx,Spectrum1,rr);
        
        A1=opConvolve3D(nx,ny,nz,Spectrum.',[1 1 1],'cyclic');
        A3=opConvolve3D(nx,ny,nz,Spectrum,[1 1 1],'cyclic');   
        M=[F3*A1,F3*B;F3*A3,F3*B];
    end
